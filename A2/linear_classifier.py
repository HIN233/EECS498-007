...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  # dW (D,C) margin(N,C) X(N,D)
  margins[margins > 0] = 1
  row_sum = torch.sum(margins, dim=1)
  margins[torch.arange(X.shape[0]), y] = -row_sum
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  indice_batch = torch.randint(0, num_train, (batch_size,))
  X_batch = X[indice_batch]
  y_batch = y[indice_batch]
  #########################################################################
  #                       END OF YOUR CODE                                #
  #########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # Replace "pass" statement with your code
  cls.train(data_dict['X_train'], data_dict['y_train'], lr, reg, num_iters, verbose=False)
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  loss = -torch.sum(torch.log(scores[torch.arange(X.shape[0]), y])) / X.shape[0] + reg * torch.sum(W * W)
  scores[torch.arange(X.shape[0]), y] -= 1
  dW = X.t().mm(scores) / X.shape[0] + 2 * reg * W
  #############################################################################
  #                          END OF YOUR CODE                                 #
  #############################################################################
